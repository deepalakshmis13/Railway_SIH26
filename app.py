...
def get_demo_users():
    ...
    # Fallback in case our dummy database
    # uses different department names.

    if (
        engineering_id is None
    ...
    if (
        trd_id is None
        and len(departments) >= 3
    ):

        trd_id = (
            departments[2]["department_id"]
        )
...
def login_screen():
    ...
    with center:
        ...
        with st.expander(
            ...
            st.write(
                "Inspection: "
                "`inspect001 / inspect123`"
            )

            st.write(
                "Control Office: "
                "`control001 / control123`"
            )
    ...
    if st.button(
        ...
        if task_created:
            ...
            st.success(
                ...
                "and added to the maintenance "
                "task queue!"
            )

            st.info(
                "The scheduling engine will "
                "consider this task during "
                "the next schedule generation."
            )
            ...
            st.warning(
                ...
                "could not be added to the existing "
                "task table."
            )
...
def control_office_dashboard():
    ...
    for route in train_route:
        ...
        st.write(
            ...
            f"KM "
            f"{route['location_start_km']} "
            f"→ "
            f"{route['location_end_km']} | "
...
def planner_dashboard():
    ...
    st.caption(
        "Centralized view of maintenance, "
        "trains and optimized blocks"
    )
    ...
    with tab2:
        ...
        if st.button(
            ...
            try:
                ...
                st.error(
                    f"Could not generate weekly plan: "
                    f"{error}"
                )
        ...
        if "weekly_plan" in st.session_state:
            ...
            st.download_button(
                ...
                label=(
                    "⬇️ Download Weekly Plan CSV"
                ),

                data=csv_data,

                file_name=(
                    "weekly_maintenance_plan.csv"
                ),
    ...
    with tab3:
        ...
        if st.button(
            ...
            try:
                ...
                st.success(
                    "Monthly plan generated successfully."
                )

            except Exception as error:

                st.error(
                    f"Could not generate monthly plan: "
                    f"{error}"
                )
        ...
        if "monthly_plan" in st.session_state:
            ...
            st.download_button(
                ...
                label=(
                    "⬇️ Download Monthly Plan CSV"
                ),

                data=csv_data,

                file_name=(
                    "monthly_maintenance_plan.csv"
                ),
    ...
    with tab4:
        ...
        st.info(
            "Simulate train delays and weather "
            "conditions without changing the "
            "database or the original Plan A."
        )


        simulation_col1, simulation_col2 = (
            st.columns(2)
        )
        ...
        if st.button(
            ...
            try:
                ...
                st.success(
                    "Plan B simulation generated."
                )

            except Exception as error:

                st.error(
                    f"Could not generate Plan B: "
                    f"{error}"
                )


        if "plan_b" in st.session_state:
            ...
            st.download_button(
                ...
                label=(
                    "⬇️ Download Plan B CSV"
                ),

                data=csv_data,

                file_name=(
                    "plan_b_simulation.csv"
                ),
